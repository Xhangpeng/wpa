...
PACKAGE_ROOT = Path('/home/ubuntu/gallery-package/western_public_work')
PACKAGE_ASSETS = PACKAGE_ROOT / 'client/public/assets'
PUBLIC_ASSETS = ROOT / 'client/public/gallery-assets'
UPLOAD_ROOT = Path('/home/ubuntu/upload')

PUBLIC_ASSETS.mkdir(parents=True, exist_ok=True)

# Copy the complete gallery image set from the provided archive. This uses filenames only;
# no attached image is opened or visually re-inspected.
for source in PACKAGE_ASSETS.iterdir():
    if source.is_file() and source.suffix.lower() in {'.jpg', '.jpeg', '.png', '.webp'}:
        target_name = source.name.replace('+', '-')
        if target_name == "SCHOOL'SLOGO.jpg":
            target_name = 'school-logo.jpg'
        shutil.copy2(source, PUBLIC_ASSETS / target_name)

# Prefer explicitly uploaded files for the matching names that arrived individually.
for name in ['accountant.jpg', 'administrator.jpg', 'gal-22.jpg']:
    uploaded = UPLOAD_ROOT / name
    if uploaded.exists():
        shutil.copy2(uploaded, PUBLIC_ASSETS / name.replace('+', '-'))

GALLERY_TSX = r'''import { useEffect, useMemo, useState } from "react";
...
(ROOT / 'client/src/pages/Gallery.tsx').write_text(GALLERY_TSX, encoding='utf-8')

# Patch App routing.
app_path = ROOT / 'client/src/App.tsx'
app = app_path.read_text(encoding='utf-8')
if 'import Gallery from "./pages/Gallery";' not in app:
    app = app.replace('import Programs from "./pages/Programs";\n', 'import Programs from "./pages/Programs";\nimport Gallery from "./pages/Gallery";\n')
if '<Route path={"/gallery"} component={Gallery} />' not in app:
    app = app.replace('      <Route path={"/programs/:slug"} component={Programs} />\n', '      <Route path={"/programs/:slug"} component={Programs} />\n      <Route path={"/gallery"} component={Gallery} />\n')
app_path.write_text(app, encoding='utf-8')

# Patch header navigation and implemented route list.
header_path = ROOT / 'client/src/components/SiteHeader.tsx'
header = header_path.read_text(encoding='utf-8')
header = header.replace('"/programs/education", "/academics/notices"', '"/programs/education", "/gallery", "/academics/notices"')
if '{ label: "Gallery", href: "/gallery" },' not in header:
    header = header.replace('  { label: "Notices", href: "/academics/notices" },\n', '  { label: "Gallery", href: "/gallery" },\n  { label: "Notices", href: "/academics/notices" },\n')
header_path.write_text(header, encoding='utf-8')

# Patch footer quick links and implemented route guard.
footer_path = ROOT / 'client/src/components/SiteFooter.tsx'
footer = footer_path.read_text(encoding='utf-8')
if '{ label: "Gallery", href: "/gallery" },' not in footer:
    footer = footer.replace('  { label: "Programs", href: "/programs" },\n', '  { label: "Programs", href: "/programs" },\n  { label: "Gallery", href: "/gallery" },\n')
footer = footer.replace('["/", "/about", "/programs", "/contact", "/apply", "/academics/notices"]', '["/", "/about", "/programs", "/gallery", "/contact", "/apply", "/academics/notices"]')
footer_path.write_text(footer, encoding='utf-8')

# Patch home gallery preview button: replace placeholder toast with direct navigation.
home_path = ROOT / 'client/src/pages/Home.tsx'
home = home_path.read_text(encoding='utf-8')
home = home.replace('import { Link } from "wouter";', 'import { Link, useLocation } from "wouter";')
if 'const [, setLocation] = useLocation();\n  return (' not in home:
    home = home.replace('function GalleryPreview() {\n  return (', 'function GalleryPreview() {\n  const [, setLocation] = useLocation();\n  return (')
home = home.replace('onClick={() => toast.info("Gallery page coming soon")}', 'onClick={() => setLocation("/gallery")}')
home_path.write_text(home, encoding='utf-8')

# Create a concise filename-to-category reference used for the integration.
summary = '''# Western Public Academy Gallery Integration Map

The uploaded Amity-style gallery archive was integrated by filename, without re-opening the attached images. Files were copied into `client/public/gallery-assets/` and the Gallery page now serves them locally from `/gallery-assets/...`.

| Category | Related filenames |
|---|---|
| Campus | `gallery-1.jpg` through `gallery-10.jpg`, `gal-11.jpg` through `gal-21.jpg`, `gal-24.jpg`, `gal-26.jpg`, `gal-27.jpg`, `gaal-28.jpg`, `gal-29.jpg`, `gal-30.jpg`, `gall-12.jpg`, `BACKGROUND-IMG-1.jpg`, `BACKGROUND-IMG-2.jpg`, `BACKGROUND-IMG-3.jpg`, `BACKGROUNDIMG4.jpg`, `home-down-cover-photo.jpg` |
| Learning | `HOTEL-MANAGEMENT.png`, `education-faculty.png`, `sports-science.jpeg` |
| Events | `gallery-9.jpg`, `gal-22.jpg`, `gal-23.jpg`, `gal-25.jpg` |
| Trips | `picnic-gallery.jpg`, `teacher-picnic.jpg`, `boating-picnic.jpg` |
| Community | `school-heads.jpg`, `principal.jpg`, `vice-principal.jpg`, `administrator.jpg`, `accountant.jpg` |

The new route is `/gallery`. It is linked from the desktop/mobile header, footer quick links, and the home-page “View Full Gallery” button.
'''
(ROOT / 'gallery-integration-map.md').write_text(summary, encoding='utf-8')

print('Gallery integration complete')
print(f'Assets copied to: {PUBLIC_ASSETS}')
print(f'Gallery assets count: {len(list(PUBLIC_ASSETS.iterdir()))}')
